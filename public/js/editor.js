...
async function loadProfileData() {
    try {
        ...
        // If a template is already selected, update the preview automatically
        if (currentTemplate) {
            updatePreview();
        }
...
function updatePreview() {
    ...
    // Proxy external images through our backend to avoid CORS/hotlinking issues
    let proxiedAvatarUrl = avatarUrl || 'https://via.placeholder.com/72';
    if (avatarUrl && !avatarUrl.startsWith('data:') && !avatarUrl.includes('/api/image-proxy')) {
        proxiedAvatarUrl = `${API_BASE_URL}/image-proxy?url=${encodeURIComponent(avatarUrl)}`;
    }
